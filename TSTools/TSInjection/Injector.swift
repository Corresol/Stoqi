...
class Injector {
    ...
    static func addInjectionRule(_ rule : InjectionRule) {
     ...
     */
    static func inject<InjectableType : Any> (_ injectable : InjectableType.Type, with parameter: Any? = nil) throws -> InjectableType {
        debugPrint(parameter.debugDescription)
...
//        if protocolType == "HomeViewModel" && targetType == "_SwiftValue" {
//            targetType = "(profile : Profile, requests : Array<RequestList>, analytic : Analytic)";
//        }
//        if protocolType == "StockViewModel" && targetType == "_SwiftValue" {
//            targetType = "Stock";
//        }
//        if protocolType == "StockProductsCategoryViewModel" && targetType == "_SwiftValue" {
//            targetType = "(category : Category, products : Array<StockProduct>)";
//        }
//        if protocolType == "StockProductViewModel" && targetType == "_SwiftValue" {
//            targetType = "StockProduct";
//        }
        
        if targetType == "_SwiftValue" && protocolType != "ProfileViewModel" && protocolType != "ProductViewModel" && protocolType != "PickerDataSource" && protocolType != "PickerItemViewModel" {
            targetType = (self.rules[protocolType]?.keys.first)!;
        }
        
        if protocolType == "ProductViewModel" && targetType == "_SwiftValue" && parameter.debugDescription.contains("Stoqi.Category") {
            targetType = "Category";
        }
        if protocolType == "ProductViewModel" && targetType == "_SwiftValue" && parameter.debugDescription.contains("Stoqi.StockProduct") {
            targetType = "StockProduct";
        }
        if protocolType == "ProductViewModel" && targetType == "_SwiftValue" && parameter.debugDescription.contains("Stoqi.ProductEntry") {
            targetType = "ProductEntry";
        }
        
        if protocolType == "PickerDataSource" && targetType == "_SwiftDeferredNSArray" && parameter.debugDescription.contains("Stoqi.Kind") {
            targetType = "Array<Kind>";
        }
        if protocolType == "PickerDataSource" && targetType == "_SwiftDeferredNSArray" && parameter.debugDescription.contains("Stoqi.Brand") {
            targetType = "Array<Brand>";
        }
        if protocolType == "PickerDataSource" && targetType == "_SwiftDeferredNSArray" && parameter.debugDescription.contains("Stoqi.Volume") {
            targetType = "Array<Volume>";
        }
        if protocolType == "PickerDataSource" && targetType == "_SwiftValue" {
            targetType = "PickerInjectionParameter";
        }
        
        if protocolType == "PickerItemViewModel" && targetType == "_SwiftValue" && parameter.debugDescription.contains("Stoqi.Volume") {
            targetType = "Volume";
        }
        if protocolType == "PickerItemViewModel" && targetType == "_SwiftValue" && parameter.debugDescription.contains("Stoqi.Brand") {
            targetType = "Brand";
        }
        if protocolType == "PickerItemViewModel" && targetType == "_SwiftValue" && parameter.debugDescription.contains("Stoqi.Kind") {
            targetType = "Kind";
        }
        
        if protocolType == "CommonHeaderViewDataSource" && targetType == "_NSContiguousString" {
            targetType = "String";
        }
        if protocolType == "CommonEmptyResultsCellDataSource" && targetType == "_NSContiguousString" {
            targetType = "String";
        }
        if protocolType == "ProfileViewModel" && targetType == "_SwiftValue" && parameter.debugDescription.contains("Stoqi.Profile") {
            targetType = "Profile";
        }
        if protocolType == "ProfileViewModel" && targetType == "_SwiftValue" && parameter.debugDescription.contains("Any.Type") {
            targetType = "Any.Type";
        }
...
struct InjectionRule : CustomStringConvertible {
    ...
    init<InjectableType> (injectable : InjectableType.Type, once : Bool = false, injection : @escaping () throws -> InjectableType) {
    ...
    /// - Parameter InjectableType: Type of injectable which must confrom to general Injectable protocol.
    /// - Parameter ParameterType: Type of the parameter which will be passed to the closure.
    init<InjectableType, TargetType> (injectable : InjectableType.Type, targetType : TargetType.Type, once : Bool = false, injection : @escaping (TargetType) throws -> InjectableType) {
    ...
    /// - Parameter injection: A closure to which type instantiation is delegated.
    /// - Parameter InjectableType: Type of injectable which must confrom to general Injectable protocol.
    init<InjectableType> (injectable : InjectableType.Type, injected : @autoclosure @escaping () throws -> InjectableType) {
